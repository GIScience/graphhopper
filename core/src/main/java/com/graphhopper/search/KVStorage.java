...
// This class is backported from upstream graphhopper/graphhopper (master branch, 2026-05-29).
// It is used in OSMNodeData to replace heap-intensive List<Map<String,Object>> for barrier
// node tag storage during OSM import. The upstream change paired KVStorage with a full
// int-to-long node ID migration; this backport uses only the KVStorage class itself, which
// is self-contained and does not require that migration.
//
// *** UPGRADE WARNING ***
// When upgrading the ORS GraphHopper fork to a GH version that already includes KVStorage
// natively (i.e., you see 'package com.graphhopper.search; class KVStorage' in upstream),
// you MUST delete this file — do NOT merge or overwrite. The upstream class should take
// precedence. Keeping both will cause a duplicate class conflict at compile time.
// Also delete Constants.VERSION_KV_STORAGE added alongside this backport.
// Affected files: KVStorage.java, Constants.java, OSMNodeData.java (marked with
// ORS-GH BACKPORT START/END).
// ORS-GH BACKPORT END
...
public class KVStorage {
    ...
    // separate last-entry tracking for addTags (plain Map<String,Object>)
    private long lastTagEntryPointer = -1;
    private Map<String, Object> lastTagEntries;
    ...
    /**
     * Writes the entries of a plain {@code Map<String, Object>} into the storage, starting after the
     ...
     */
    private long setTagsList(long currentPointer, final Map<String, Object> tags) {
